...
{
    bin=$1
    if ! objdump -s -j .gnu_debuglink $bin | grep debuglink
    then
...
EOF
        ...
        do
            wget $wget_url -O $obproxy_yum_filename
        ...
        then
            ...
            then
                ...
                echo "obrpxoy版本:"
                echo `$root_path/obproxy -V`
            else
                echo "obproxy yum源文件解压失败, 沿用原有FARM逻辑"
            fi
        else
            echo "obproxy yum源文件下载失败, 沿用原有FARM逻辑"
        fi
    fi
    ...
    test -f liboblog.so.1.0.0 && (strip libobserver.so.0.0.0 || return 5)

    # In some branch, agentserver&incbackupserver may not be
    # created. Thus to create them if they don't exist.
    touch agentserver
    touch incbackupserver
